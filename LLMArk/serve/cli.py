...
def main(args):
    ...
    image_tensor = image_processor.preprocess(image, return_tensors="pt")["pixel_values"].half().cuda()
        ...
        # YOLO processing images
    # Load a model
    yolo_model = YOLO("ultralytics8/runs/detect/train/weights/best.pt")  # pretrained YOLO11n model
    
    results = yolo_model(args.image_file, conf=0.4)
    ...
    for box in boxes:
        ...
        zb = [round(x1/640, 3), round(y1/640, 3), round(x2/640, 3), round(y2/640, 3)]
        all_boxes.append(zb)
    print("======Yolo boxes: "+str(all_boxes))
    ...
    while True:
        ...
        prompt_boxes = get_boxes(prompt)
        
        if len(prompt_boxes) == 0:
            prompt = prompt.replace("in",f"at position {str(all_boxes)} in")
